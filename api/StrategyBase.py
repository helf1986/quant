...
#-----------------eventprocess函数-----------------    
    ...
    def buy(self,unit,price):
        ...
        if self.MarketPosition==-1:   
            ...
            print ("pre_buytocover(%s,%s)"%(unit,price) )
#            if type_=='realtime':
#                if self.module=='usdt':order=self.api.marginsec_close(exchange='huobipro', source='margin-api', sec_id='btcusdt', price=0, volume=unit,marginorder_id=self.id)              
...
#                self.id=order.order_id   
    ...
    def sellshort(self,unit,price):
        ...
        if self.MarketPosition==1:   
            self.MarketPosition=0;
            print ("pre_sell(%s,%s)"%(unit,price) )
#            if type_=='realtime':
#                if self.module=='usdt':order=self.api.close_long(exchange='huobipro', source='margin-api', sec_id='btcusdt', price=0, volume=unit)  
#                if self.module=='btc':order=self.api.margincash_close(exchange='huobipro', source='margin-api', sec_id='btcusdt', price=0, volume=unit,marginorder_id=self.id)  
       
        if self.MarketPosition==0:
            self.MarketPosition=-1;
            print ("sellshort(%s,%s)"%(unit,price))
#            if type_=='realtime':
...
#                self.__orderqueue.put(order)  
    def buytocover(self,unit,price):
        ...
        if self.MarketPosition==-1:
            self.MarketPosition=0;
            print ("buytocover(%s,%s)"%(unit,price))
#            if type_=='realtime':
#                if self.module=='usdt':order=self.api.marginsec_close(exchange='huobipro', source='margin-api', sec_id='btcusdt', price=0, volume=unit,marginorder_id=self.id)              
#                if self.module=='btc':order=self.api.close_short(exchange='huobipro', source='margin-api', sec_id='btcusdt', price=0, volume=unit)              
#                self.__orderqueue.put(order)  
