...
data = btc_data[['open', 'high', 'low', 'close']].iloc[0:60 * N:60].copy()
...
for each in btc_data.index[60 * N::60]:
    ...
    # 向上突破，买入
    if (data.loc[last, 'close'] < data.loc[last, 'upper']) and (data.loc[now, 'close'] > data.loc[now, 'upper']):
    ...
    # 向下突破，卖出
    elif (data.loc[last, 'close'] > data.loc[last, 'lower']) and (data.loc[now, 'close'] < data.loc[now, 'lower']):
        data.loc[now, 'signal'] = -1
